/**
 * GET  /api/v1/flows — List flows for the current user's org
 * POST /api/v1/flows — Create a new flow (admin only)
 */

import { NextRequest, NextResponse } from 'next/server';
import { getCurrentUser, requireRole } from '@/lib/auth/roles';
import { listFlows, createFlow, type CreateFlowInput } from '@/services/flows/flow.service';
...
export async function GET() {
  ...
  const result = await listFlows(userResult.data.org_id);
  if (!result.success) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = (await request.json()) as CreateFlowInput;
    const result = await createFlow(userResult.data.org_id, userResult.data.id, body);
    ...
    return NextResponse.json(toApiResponse(failure('INTERNAL_ERROR', 'Failed to create flow')), { status: 500 });
  }
}
